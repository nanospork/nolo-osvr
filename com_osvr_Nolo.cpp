...
#include "btea.c"
	...
	private:
		...
		float lerpControllerRange(int input, float minInput, float maxInput) {
			if (input == 0 || input == 255) { return 0; }
			else if (input <= minInput) { return -1; }
			else if (input >= maxInput) { return 1; }
			else {
				float val = ((float)input - minInput) * 2 / (maxInput - minInput);
				return -1 + fmax(0, fmin(2, val));
			}
		}
		void decodeControllerCV1(int idx, unsigned char *data) {
			...
			if (data[3 + 3 * 2 + 4 * 2 + 1]) {  // Only report touch if there is one
				axis_value = lerpControllerRange(data[3 + 3 * 2 + 4 * 2 + 2], 38, 217);
				// invert axis
				axis_value *= -1;
				osvrDeviceAnalogSetValueTimestamped(m_dev, m_analog, axis_value, idx*NUM_AXIS + 0, &m_lastreport_time);
				axis_value = lerpControllerRange(data[3 + 3 * 2 + 4 * 2 + 2 + 1], 38, 217);
				axis_value *= -1;
